...
def connect_platform_to_calendar(calendar_id):
    ...
    try:
        ...
        calendar_result = supabase.table('calendars').select('*').eq('owner_id', user_id).eq('id', calendar_id).execute()
...
def get_connections_summary():
    ...
    try:
        ...
        # Get calendars count
        calendars_result = supabase.table('calendars').select('id').eq('owner_id', user_id).execute()
