...
try:
    ...
    app.register_blueprint(calendar_selection_bp)
    print("[SUCCESS] Calendar selection routes registered")
except ImportError as e:
...
def sync_calendar():
    ...
    try:
        ...
        # Notion에서 NotionFlow로 이벤트 가져오기 (Import)
        ...
        # 2. 기존 NotionFlow 일정을 외부 플랫폼으로 내보내기 (Export)
...
def import_events_from_notion(user_id: str, calendar_id: str) -> int:
    """Notion에서 이벤트를 가져와서 NotionFlow calendar_events 테이블에 저장"""
    import json
    import logging
...
def import_events_from_google(user_id: str, calendar_id: str) -> int:
    """Google Calendar에서 이벤트를 가져와서 NotionFlow calendar_events 테이블에 저장"""
    import json
    import logging
...
def mark_platform_connected(platform):
    ...
    try:
        ...
        # Get request data for notionflow_calendar_id if provided
        data = request.get_json() or {}
        ...
        if platform == 'google':
            try:
                ...
                # Add notionflow_calendar_id if provided
                if notionflow_calendar_id:
                    config_data['calendar_id'] = notionflow_calendar_id
                ...
                if existing_config.data:
                    ...
                    if notionflow_calendar_id:
                        update_data['calendar_id'] = notionflow_calendar_id
