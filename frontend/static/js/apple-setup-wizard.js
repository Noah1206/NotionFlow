...
class AppleSetupWizard {
    ...
    /**
     * NotionFlow 캘린더 선택 모달 표시 (구글/노션과 동일한 디자인)
     */
    async showCalendarSelectionModal() {
        try {
            ...
            // 구글/노션과 동일한 캘린더 리스트 HTML 생성
            const calendarItems = calendars.map(cal => `
                <div class="calendar-item" onclick="window.appleWizard.selectCalendarForSync('${cal.id}')">
            ...
            // 구글/노션과 동일한 모달 HTML 구조
            const modalHTML = `
                <div class="modal-overlay" id="apple-calendar-modal">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h2>Apple Calendar 연동</h2>
                            <button class="modal-close" onclick="window.appleWizard.closeModal('apple-calendar-modal')">&times;</button>
                        </div>
                        <div class="modal-body">
                            <p><strong>Apple Calendar</strong>과 동기화할 NotionFlow 캘린더를 선택하세요:</p>
    ...
    /**
     * 캘린더 선택 후 연결 수행 (구글/노션과 동일한 플로우)
     */
    async selectCalendarForSync(calendarId) {
        try {
            // 모달 닫기
            this.closeModal('apple-calendar-modal');

            // 연결 API 호출 (구글/노션과 동일한 방식)
            const response = await fetch('/api/platform/apple/connect', {
                method: 'POST',
            ...
            if (result.success) {
                // 성공 알림
                this.showNotification(`Apple Calendar이 성공적으로 연결되었습니다!`, 'success');
