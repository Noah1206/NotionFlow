...
function initializeCalendar() {
    // Set month view as default (user expects to see month calendar)
    currentView = 'month';
    ...
    if (monthBtn) {
        ...
        // Fallback to week if month button doesn't exist
    ...
    // Force render calendar grid - override any GoogleCalendarGrid
    setTimeout(() => {
        console.log('🚀 Initializing calendar view:', currentView);

        // Clear any existing content in the container first
        const mainGrid = document.getElementById('calendar-grid-container');
        if (mainGrid) {
            mainGrid.innerHTML = '';
            console.log('🧹 Cleared calendar container for fresh render');
        }
        ...
        updateStats();
    }, 150); // Slightly longer delay to ensure GoogleCalendarGrid finishes first
}
...
function renderMonthView() {
    ...
    // Clear any existing content to avoid conflicts with GoogleCalendarGrid
    const mainGrid = document.getElementById('calendar-grid-container');
    if (mainGrid) {
        mainGrid.innerHTML = '';
        console.log('🧹 Cleared calendar container for month view');
    }
...
function renderMainCalendar() {
    ...
    // Use the correct calendar container ID from template
    const mainGrid = document.getElementById('calendar-grid-container');

    if (!mainGrid) {
        console.error('❌ Calendar grid container not found - ID: calendar-grid-container');
        console.error('Available containers:',
            Array.from(document.querySelectorAll('[id*="calendar"]')).map(el => el.id));
        return;
    }

    console.log('🗓️ Rendering month calendar for:', year, month + 1);
    ...
    // Create month calendar HTML structure
    mainGrid.innerHTML = `
        <div class="month-calendar-wrapper" style="height: 100%; display: flex; flex-direction: column;">
            <div class="month-header" style="padding: 16px; border-bottom: 1px solid #e5e7eb; text-align: center;">
                <h2 style="margin: 0; font-size: 20px; font-weight: 600; color: #111827;">
                    ${year}년 ${month + 1}월
                </h2>
            </div>
            <div class="month-weekdays" style="display: grid; grid-template-columns: repeat(7, 1fr); background: #f8fafc; border-bottom: 1px solid #e5e7eb;">
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #dc2626;">일</div>
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #374151;">월</div>
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #374151;">화</div>
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #374151;">수</div>
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #374151;">목</div>
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #374151;">금</div>
                <div style="padding: 12px; text-align: center; font-weight: 500; color: #dc2626;">토</div>
            </div>
            <div id="month-days-grid" class="month-days-grid" style="flex: 1; display: grid; grid-template-columns: repeat(7, 1fr); grid-template-rows: repeat(6, 1fr); gap: 1px; background: #e5e7eb;">
            </div>
        </div>
    `;

    // Get the days grid container
    ...
    // Generate all 42 days (6 weeks × 7 days)
    const startDate = new Date(year, month, 1);
    startDate.setDate(1 - startingDayOfWeek); // Go back to start of calendar week
    ...
    console.log('✅ Month calendar rendered successfully:', {
        totalDays: 42,
        year: year,
        month: month + 1,
        containerHeight: mainGrid.offsetHeight
    });
}
