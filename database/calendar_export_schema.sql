...
CREATE TABLE IF NOT EXISTS calendar_export_queue (
    ...
    -- 인덱스용 컬럼
    is_processed BOOLEAN GENERATED ALWAYS AS (export_status IN ('completed', 'failed')) STORED
);
...
CREATE TABLE IF NOT EXISTS calendar_export_logs (
    ...
    -- 통계용 컬럼
    date_created DATE GENERATED ALWAYS AS (DATE(started_at)) STORED
);
...
CREATE INDEX IF NOT EXISTS idx_export_queue_processing ON calendar_export_queue(user_id, is_processed) WHERE NOT is_processed;
